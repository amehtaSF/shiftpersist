...
NUMERIC_COLS <- c("erq", "ace", "srh", "gender", "pss", "brs", "twn",
                  "twn_freq", "fear_covid", "age", "msss_us")
```
...
df_proc <- df_raw %>% 
  ...
  mutate_at(vars(one_of(NUMERIC_COLS)), as.numeric) 
...
df_pid %>% 
  ...
  mutate(
    # twn_group = case_when(
    #   twn >= (mean(twn) + sd(twn)) ~ "+1 MAD",
    #   twn <= (mean(twn) - sd(twn)) ~ "-1 MAD",
    #   twn < (mean(twn) + sd(twn)) & twn > (mean(twn) - sd(twn)) ~ "Within 1 MAD"
    # ),
    twn_group = case_when(
      twn > quantile(twn, .66) ~ ">66 percentile",
      twn < quantile(twn, .33)  ~ "<33 percentile",
      twn <= quantile(twn, .66) & twn >= quantile(twn, .33) ~ "33-66 percentile"
    )) %>% 
