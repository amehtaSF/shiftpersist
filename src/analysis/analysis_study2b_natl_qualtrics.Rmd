...
```{r, message=F}
cor.test(df_proc$nature_freq, df_proc$panas_neg)
...
```{r, message=F}
cor.test(df_proc$nature_freq, df_proc$sitselect_approach)
...
```{r, message=F}
cor.test(df_proc$nature_freq, df_proc$pmerq_attdep_dis)
...
```{r, message=F}
cor.test(df_proc$nature_freq, df_proc$pmerq_reappraise)
...
```{r, message=F}
cor.test(df_proc$nature_freq, df_proc$rrs_brood)
...
```{r, message=F}
cor.test(df_proc$nature_freq, df_proc$fm)
...
```{r, message=F}
cor.test(df_proc$nature_freq, df_proc$swl)
...
```{r, message=F}
cor.test(df_proc$nature_freq, df_proc$pss)
...
```{r, message=F}
cor.test(df_proc$nature_freq, df_proc$panas_pos)
...
```{r, message=F}
df_proc %>% 
  dplyr::mutate_if(is.numeric, ~as.vector(unlist(scale(.)))) %>%
...
df_proc %>% 
  dplyr::mutate_if(is.numeric, ~as.vector(unlist(scale(.)))) %>%
  lm(panas_pos ~ nature_freq * high_ace, .) %>% 
  summary
df_proc %>% 
  ggplot(aes(x = nature_freq, y = panas_pos, color = high_ace)) + 
  geom_point(alpha = .3) + 
  geom_smooth(method = "lm") +
...
df_proc %>% 
  dplyr::mutate_if(is.numeric, ~as.vector(unlist(scale(.)))) %>%
  lm(fm ~ nature_freq * high_ace, .) %>% 
  summary
df_proc %>% 
  ggplot(aes(x = nature_freq, y = fm, color = high_ace)) + 
  geom_point(alpha = .3) + 
  geom_smooth(method = "lm") +
...
df_proc %>%
  dplyr::mutate_if(is.numeric, ~as.vector(unlist(scale(.)))) %>%
  lm(swl ~ nature_freq * high_ace, .) %>%
  summary
df_proc %>%
  ggplot(aes(x = nature_freq, y = swl, color = high_ace)) +
  geom_point(alpha = .3) +
  geom_smooth(method = "lm") +
...
df_proc %>% 
  dplyr::mutate_if(is.numeric, ~as.vector(unlist(scale(.)))) %>%
  lm(panas_neg ~ nature_freq * high_ace, .) %>% 
  summary
df_proc %>% 
  ggplot(aes(x = nature_freq, y = panas_neg, color = high_ace)) + 
  geom_point(alpha = .3) + 
  geom_smooth(method = "lm") +
...
df_proc %>% 
  dplyr::mutate_if(is.numeric, ~as.vector(unlist(scale(.)))) %>%
  lm(pss ~ nature_freq * high_ace, .) %>% 
  summary
df_proc %>% 
  ggplot(aes(x = nature_freq, y = pss, color = high_ace)) + 
  geom_point(alpha = .3) + 
  geom_smooth(method = "lm") +
