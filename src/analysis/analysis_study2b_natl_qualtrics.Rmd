...
try_param <- function(p, default){
  tryCatch(p,
           error=function(e){
             print(glue("Warning: could not find {rlang::quo_name(rlang::enquo(p))}"))
             print(glue("Defaulting to {default}"))
             return(default)
           })
}
```
...
## Read data
```{r, message=F}
data_filepath <- try_param(params$inputfile,"data/proc/national_study_2b_proc.csv")
